...
# 1. Replace the header HTML
# We find the specific <header class="form-header"> block that contains 'logo_url'
pattern = r'              <header class="form-header">\s*<div class="form-logo-container">.*?<textarea v-model="generatedForm.description" class="form-description-input" placeholder="Form Description"></textarea>\s*</header>'
              ...
              <header class="form-header" style="display: flex; align-items: flex-start; gap: 24px; position: relative; text-align: left;">
                <div class="form-logo-widget" style="position: relative; resize: both; overflow: hidden; width: 120px; height: 120px; min-width: 80px; min-height: 80px; max-width: 300px; max-height: 300px; border: 2px dashed #cbd5e1; border-radius: 12px; box-shadow: 0 0 15px rgba(56, 189, 248, 0.25); background: #f8fafc; display: flex; flex-direction: column; align-items: center; justify-content: center; cursor: move; flex-shrink: 0;">
                  ...
                  <div class="form-logo-placeholder" v-else style="display: flex; flex-direction: column; align-items: center; gap: 6px; padding: 12px;">
                    ...
                    <span style="font-size: 11px; color: #64748b; text-align: center; font-weight: 600; line-height: 1.2;">Upload / Replace<br/>Logo</span>
                  </div>
...
if match:
    ...
    print("Failed to match HTML block!")

# 2. Update CSS rules to make sure .form-logo-widget overlay works
new_hover = '''
...
# 3. Clean up any center alignment in .form-header in AdminView.vue CSS
text = re.sub(r'(\.form-header\s*\{[^}]*)text-align:\s*center;([^}]*\})', r'\1\2', text)
